 ...
 *
 *  resource_ref_id:
 *  				the resource which can be used by the task,
 *  				-1 if the task does not access any resource
 *
 *  resource:		the resource used by the task,
 *  				NULL if the task is not locking the resource at the moment
 *
 *  mrsp_status:	define the status of the task related to resource sharing
 *  	NO_RESOURCE
 *  	WAITING_RESOURCE
 *  	HOLDING_RESOURCE
 *
 *  helper_task:    points to the task that is scheduled, waiting for
 *               	the resource retained by this task and helping
 *
 *  helping_task:   points to the task which is helped by this task
 *  waiting_ticket: the ticket the task is waiting for when try to lock a resource
 */
...
struct rt_task {
	...
	struct task_struct	*helping_task;
	int			mrsp_ceiling;
	int         waiting_ticket;
