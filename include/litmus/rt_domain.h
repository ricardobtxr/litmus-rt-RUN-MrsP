...
#define WANT_DEBUG_RTDOMAIN_EVENTS
...
static inline int  is_queued(struct task_struct *t)
{
	...
	task_is_queued = bheap_node_in_heap(tsk_rt(t)->heap_node);
	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	TRACE("rt_domain.is_queued(%d)=%d\n", t->pid, task_is_queued);
	#endif

	return task_is_queued;
}

static inline void  remove(rt_domain_t* rt, struct task_struct *t)
{
	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	TRACE("rt_domain.remove(%d)\n", t->pid);
	#endif
...
static inline struct task_struct*  take_ready(rt_domain_t* rt)
{
	...
	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	if (ret)
		TRACE("rt_domain.take_ready() = %d\n", ret->pid);
	else
		TRACE("rt_domain.take_ready() = NULL\n");
	#endif
...
{
	int ret = !bheap_empty(&rt->ready_queue);

	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	TRACE("rt_domain.__jobs_pending() = %d\n", ret);
	#endif
...
static inline int  jobs_pending(rt_domain_t* rt)
{
	...
	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	TRACE("rt_domain.jobs_pending() = %d\n", ret);
	#endif
