...
#define LITMUS_TRACE_MRSP_PREFIX "%d|P%d|"
#define LITMUS_TRACE_MRSP_ARGS  atomic_add_return(1, &__log_seq_no),	\
		raw_smp_processor_id()
...
#define LITMUS_TRACE_MRSP(fmt, args...)						\
	sched_trace_log_message(LITMUS_TRACE_MRSP_PREFIX fmt,			\
			LITMUS_TRACE_MRSP_ARGS,  ## args)
...
// 0 for removing the traces while executing the experiments, 1 for maintaining the traces
#if 0
#define TRACE(fmt, args...) LITMUS_TRACE(fmt, ## args)
#define TRACE_MRSP(fmt, args...) LITMUS_TRACE_MRSP(fmt, ## args)
