...
#define WANT_CORE_LITMUS_EVENTS
...
{
	unsigned long flags;
	int cpu, success = 0, debug_state, debug_pstate;

	#ifdef WANT_CORE_LITMUS_EVENTS
	if (is_realtime(p))
...
out:
	#ifdef WANT_CORE_LITMUS_EVENTS
	if (is_realtime(p) && !(debug_pstate & debug_state))
		TRACE_TASK(p, "try_to_wake_up() done state:%d, success=%d, parameter(state)=%d\n", debug_pstate, success, debug_state);
	#endif
...
recheck:
	...
	if (policy == SCHED_LITMUS) {
		...
		#ifdef WANT_CORE_LITMUS_EVENTS
		TRACE("__sched_setscheduler().if(policy == SCHED_LITMUS), %d\n", p->pid);
		#endif
...
change:
	...
	if (is_realtime(p)) {
		...
		#ifdef WANT_CORE_LITMUS_EVENTS
		TRACE("__sched_setscheduler().if(is_realtime(p)), %d\n", p->pid);
		#endif
	...
	if (litmus_policy(policy)) {
		...
		#ifdef WANT_CORE_LITMUS_EVENTS
		TRACE("__sched_setscheduler().if(litmus_policy(policy)), %d\n", p->pid);
		#endif
