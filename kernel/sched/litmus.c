...
static void __attribute__((optimize("O0"))) update_time_litmus(struct rq *rq, struct task_struct *p)
{
	...
	if (delta) {
		...
		/*TRACE_TASK(p, "charged %llu exec (tot:%llu, rem:%llu), serv %d\n",
			delta, p->rt_param.job_params.exec_time, budget_remaining(p),
			p->rt_param.task_params.run_server_id);*/
		#endif
	}
	/* sched_clock() */
...
__attribute__((optimize("O0"))) litmus_schedule(struct rq *rq, struct task_struct *prev)
{
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	if (next && next != prev && is_realtime(next))
		TRACE_TASK(next, "Task scheduled into CPU #%d instead of task %d\n", rq->cpu, prev?prev->pid:-1);
	#endif
	...
	if (next && task_rq(next) != rq) {
		...
		#ifdef WANT_LITMUS_DEBUG_EVENTS
		TRACE_TASK(next, "migrate from CPU #%d to #%d\n", from_where, rq->cpu);
		#endif

		/* while we drop the lock, the prev task could change its state
		 */
		BUG_ON(prev != current);
...
static void __attribute__((optimize("O0"))) enqueue_task_litmus(struct rq *rq, struct task_struct *p,
				int flags)
{
	TRACE_TASK(p, "Task going to wake up. tsk_rt(p)->present = 1\n");
	tsk_rt(p)->present = 1;
...
static void __attribute__((optimize("O0"))) dequeue_task_litmus(struct rq *rq, struct task_struct *p,
				int flags)
{
...
#endif
		TRACE_TASK(p, "Task going to sleep. tsk_rt(p)->present = 0\n");
		litmus->task_block(p);
		tsk_rt(p)->present = 0;
...
static void __attribute__((optimize("O0"))) yield_task_litmus(struct rq *rq)
{
	TS_SYSCALL_IN_START;
	TS_SYSCALL_IN_END;
 ...
 */
static __attribute__((optimize("O0"))) struct task_struct *pick_next_task_litmus(struct rq *rq,
...
static void __attribute__((optimize("O0"))) task_tick_litmus(struct rq *rq, struct task_struct *p, int queued)
{
 ...
 */
static void __attribute__((optimize("O0"))) set_curr_task_litmus(struct rq *rq)
{
...
static void __attribute__((optimize("O0"))) update_curr_litmus(struct rq *rq)
{
