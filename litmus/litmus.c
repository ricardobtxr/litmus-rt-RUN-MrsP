...
#define WANT_LITMUS_DEBUG_EVENTS
...
asmlinkage long sys_set_rt_task_param(pid_t pid, struct rt_task __user * param)
{
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE("Litmus.sys_set_rt_task_param: Setting up rt task parameters for process %d.\n", pid);
	#endif

	if (pid < 0 || param == 0) {

		#ifdef WANT_LITMUS_DEBUG_EVENTS
        TRACE("Litmus.sys_set_rt_task_param: (pid < 0 || param == 0)\n");
		#endif
	...
	if (copy_from_user(&tp, param, sizeof(tp))) {
		...
		#ifdef WANT_LITMUS_DEBUG_EVENTS
        TRACE("Litmus.sys_set_rt_task_param: copy_from_user\n");
		#endif
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE("Litmus.sys_set_rt_task_param: read_lock_irq(&tasklist_lock)\n");
	#endif

	rcu_read_lock();

	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE("Litmus.sys_set_rt_task_param: rcu_read_lock()\n");
	#endif

	if (!(target = find_task_by_vpid(pid))) {
		#ifdef WANT_LITMUS_DEBUG_EVENTS
        TRACE("Litmus.sys_set_rt_task_param: !(target = find_task_by_vpid(pid))\n");
		#endif
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE("Litmus.sys_set_rt_task_param: rcu_read_unlock\n");
	#endif
	...
	{
		#ifdef WANT_LITMUS_DEBUG_EVENTS
		TRACE(KERN_INFO "litmus.sys_set_rt_task_param: real-time task %d rejected "
	...
	if (is_realtime(target)) {
		 ...
		 */
		#ifdef WANT_LITMUS_DEBUG_EVENTS
        TRACE("Litmus.sys_set_rt_task_param: is_realtime(target)\n");
		#endif
		...
		#ifdef WANT_LITMUS_DEBUG_EVENTS
        TRACE("Litmus.sys_set_rt_task_param: else (is_realtime(target))\n");
		#endif
        ...
        out_unlock:
			#ifdef WANT_LITMUS_DEBUG_EVENTS
        	TRACE("Litmus.sys_set_rt_task_param: read_unlock_irq(&tasklist_lock)\n");
			#endif

            read_unlock_irq(&tasklist_lock);
        out:
			#ifdef WANT_LITMUS_DEBUG_EVENTS
			TRACE("Litmus.sys_set_rt_task_param: out\n");
			#endif
...
asmlinkage long  sys_complete_job(void)
{
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE_TASK(current, "Before complete_job() is_RT:%d period:%d at %llu\n", is_realtime(current), get_rt_period(current), litmus_clock());
	#endif
...
asmlinkage long  sys_wait_for_job_release(unsigned int job)
{
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE_TASK(current, "sys_wait_for_job_release() is_RT:%d period:%d at %llu\n", is_realtime(current), get_rt_period(current), litmus_clock());
	#endif
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE("sys_wait_for_job_release(), after WHILE at %llu\n", litmus_clock());
	#endif
...
static long  __litmus_admit_task(struct task_struct* tsk)
{
	...
	if (!tsk_rt(tsk)->heap_node || !tsk_rt(tsk)->rel_heap) {
		TRACE(KERN_WARNING "litmus: no more heap node memory!?\n");
	...
	#ifdef WANT_ALL_SCHED_EVENTS
	TRACE("litmus->admit_task(%d)\n", tsk->pid);
	#endif

	err = litmus->admit_task(tsk);

	#ifdef WANT_ALL_SCHED_EVENTS
	TRACE("ret litmus->admit_task(%d) = %d\n", tsk->pid, err);
	#endif
...
static int __do_plugin_switch(struct sched_plugin* plugin)
{
	...
	if (atomic_read(&rt_task_count) == 0) {
		...
		if (0 != ret) {
			TRACE(KERN_INFO "Can't activate %s (%d).\n",
			       plugin->plugin_name, ret);
			plugin = &linux_sched_plugin;
		}

		TRACE(KERN_INFO "Switching to LITMUS^RT plugin %s.\n", plugin->plugin_name);
		litmus = plugin;
	} else
		ret = -EBUSY;
out:
				...
				unsigned long unused2, void *unused3)
{
	...
	if (litmus != &linux_sched_plugin) {
		...
		if (ret) {
			TRACE("Auto-shutdown of active Litmus plugin failed.\n");
		}
	}
	return NOTIFY_DONE;
}
...
static int __init _init_litmus(void)
{
	 ...
	 *      operation.
	 */
	TRACE("Starting LITMUS^RT kernel\n");
	...
	if (!register_sysrq_key('x', &sysrq_kill_rt_tasks_op))
		TRACE("Registered kill rt tasks magic sysrq.\n");
	else
		TRACE("Could not register kill rt tasks magic sysrq.\n");
#endif
