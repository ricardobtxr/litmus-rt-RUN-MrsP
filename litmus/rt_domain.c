 ...
 *						ns_to_ktime(rh->release_time),
 *						0, HRTIMER_MODE_ABS_PINNED, 1); //PINNED?
 */
...
void  __add_ready(rt_domain_t* rt, struct task_struct *new)
{
	...
	TRACE_TASK(new, "__add_ready(C:%llu P:%llu D:%llu r:%llu) at %llu\n",
...
{
	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	TRACE_TASK(task, "__add_release_on(), rel=%llu, target=%d\n",
...
{
	#ifdef WANT_DEBUG_RTDOMAIN_EVENTS
	TRACE_TASK(task, "__add_release(), rel=%llu, ddl=%llu now=%llu\n", get_release(task), get_deadline(task), litmus_clock());
	#endif
