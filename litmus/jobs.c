...
static inline void  setup_release(struct task_struct *t, lt_t release)
{
	...
	TRACE_TASK(t, "setup_release(%llu), deadline=%llu, now=%llu\n", release, t->rt_param.job_params.deadline, litmus_clock());
	#endif
...
long  default_wait_for_release_at(lt_t release_time)
{
	...
	#ifdef WANT_LITMUS_DEBUG_EVENTS
	TRACE_TASK(t, "default_wait_for_release_at(%llu). Sporadic:%d\n", release_time, tsk_rt(t)->sporadic_release);
	#endif
