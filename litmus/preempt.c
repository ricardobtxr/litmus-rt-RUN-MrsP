...
void sched_state_will_schedule(struct task_struct* tsk)
{
	...
	if (likely(task_cpu(tsk) == smp_processor_id())) {
		...
		if (is_in_sched_state(TASK_PICKED | PICKED_WRONG_TASK)) {
			...
			#ifdef CONFIG_PREEMPT_STATE_TRACE2
			if (is_realtime(tsk))
				TRACE_TASK(tsk, "CALL set_sched_state(PICKED_WRONG_TASK);\n");
			#endif
			...
			#ifdef CONFIG_PREEMPT_STATE_TRACE2
			if (is_realtime(tsk))
				TRACE_TASK(tsk, "CALL set_sched_state(WILL_SCHEDULE);\n");
			#endif
...
void sched_state_ipi(void)
{
	...
	if (is_in_sched_state(SHOULD_SCHEDULE)) {
		...
		if (is_realtime(current))
			TRACE_TASK(current, "RECEIVE sched_state_ipi();\n");
		#endif
...
void litmus_reschedule(int cpu)
{
	...
	#ifdef CONFIG_PREEMPT_STATE_TRACE2
	if (is_realtime(current))
		TRACE_TASK(current, "litmus_reschedule(#%d); picked-ok:%d sched-ok:%d\n", cpu,
