...
/* Adapting RUN alghorithm to new kernel, Ricardo
 * commented this function because it does not exist in the new version of litmus
feather_callback void save_cpu_timestamp(unsigned long event)
{
	write_cpu_timestamp(event, 0,
			    current->pid,
			    0, LOCAL_IRQ_COUNT, 0,
			    0, RECORD_LOCAL_TIMESTAMP);
}*/
